...
from django.contrib.auth import authenticate,login , logout  # user authentication , login,logout
...
@login_required(login_url='login_view')   # login required
def index(request):
    if request.method == "POST":
...
def register_view(request):
    if request.method == "POST":
        ...
        user.set_password(password)   # for encryped password
        user.save()
